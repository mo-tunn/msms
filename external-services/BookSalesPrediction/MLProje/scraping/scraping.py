...
if __name__ == "__main__":
    ...
    base_url = "https://www.kitapyurdu.com/index.php?route=product/best_sellers&sort=rating_count&order=DESC&list_id=6&limit=100&page={}"
    
    all_books_data = []; book_urls = set()

    print("Kitap linkleri toplanıyor...")
    # **DEĞİŞİKLİK: 1000 kitap için 10 sayfa gezilecek (10 sayfa * 100 kitap/sayfa).**
    for page_num in range(1, 11): 
        if len(book_urls) >= 1000: break
        try:
    ...
    for url in list(book_urls):
        ...
        if count >= 1000: 
            print("Hedeflenen 1000 kitap verisine ulaşıldı. İşlem durduruluyor.")
            break
    ...
    if all_books_data:
        ...
        df.to_csv('kitapyurdu_veriseti.csv', index=False, encoding='utf-8-sig')
        print("\nİşlem tamamlandı! 'kitapyurdu_veriseti.csv' dosyası oluşturuldu.")
    else:
